(function attachPlaylogEngine(root, factory) {
  ...
  const QUICK_EVALUATION_FIELDS = {
    common: [
      { key: "activity", label: "활동량", description: "움직임·압박·체력", mapsTo: ["activity", "stamina"] },
      { key: "pass", label: "패스", description: "패스·빌드업", mapsTo: ["stablePass", "buildUp"] },
      { key: "ballControl", label: "볼 컨트롤", description: "터치·소유 안정성", mapsTo: ["firstTouch", "dribbleImpact"] },
      { key: "decision", label: "판단력", description: "선택·의사결정", mapsTo: ["decision", "composure"] },
      { key: "gameUnderstanding", label: "경기 이해도", description: "위치선정·경기 읽기", mapsTo: ["offTheBall", "concentration"] },
    ],
    positionPerformance: { key: "positionPerformance", label: "포지션 수행도", description: "포지션 역할 수행" },
  };
  const LABELS = {
